...
```{r}
path_stage2_4_df<- surv_brca_cli|> select(time,vital_status,pathologic_stage)|> filter(pathologic_stage == "Stage II" | pathologic_stage == "Stage IV")

fit24 <- survfit(Surv(time,vital_status)~pathologic_stage, data = path_stage2_4_df)
LogRank23<- survdiff(Surv(time,vital_status)~pathologic_stage, data = path_stage2_4_df)
...
km_fit_path_stageIV <- survfit(Surv(time= time, event=vital_status) ~ 1, data = subset(surv_brca_cli, pathologic_stage == "Stage IV"))
